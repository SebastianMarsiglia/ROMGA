...
picam2 = Picamera2()
config = picam2.create_preview_configuration(main={"size": (1280, 720)})  # 640, 480 ; 1280, 720 ; 1920, 1080
...
def parse_feedback(data):
    ...
    if start == START_FRAME and calc_checksum == checksum:
        ...
        print(f"Volt = {batVoltage/100:.1f}, Steer = {cmd1}, Speed = {cmd2}, RPM Der = {speedR_meas}, RPM Izq = {speedL_meas}, km/h = {((speedL_meas-speedR_meas)/2)*0.0311:.1f}, Temp = {boardTemp/10}")
...
def continuous_command_loop():
    global steerjoy, speedjoy, last_send_time, batVoltage
    while True:
...
if __name__ == '__main__':
    try:
        ...
        socketio.run(app, host='192.168.1.25', port=4567) #Importante verificar la IP de la red a la que esta conectada la RPi y el celular
